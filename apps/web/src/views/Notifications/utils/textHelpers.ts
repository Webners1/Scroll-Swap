...
export const getOnBoardingDescriptionMessage = (isOnBoarded: boolean, t: TranslateFunction) => {
  let onBoardingDescription: string = t(
    'Finally, subscribe to notifications to stay informed on the latest news and updates that LolPadSwap has to offer.',
  )
  if (!isOnBoarded) {
    onBoardingDescription = t(
      'Get started with notifications from LolPadSwap. First authorize notifications by signing in your wallet',
    )
  }
